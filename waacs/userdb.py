...
class UserDB(object):
    ...
    def create_user(self):
        ...
        while True:
            #10文字のユーザ名とパスワードの生成
            user_id = "".join([random.choice(self.SOURCE_CHAR) for x in xrange(10)])
            if not self.exists_user(user_id):
                break
        password = "".join([random.choice(self.SOURCE_CHAR) for x in xrange(10)])
        logger.debug("create user: %s : %s", user_id, password)
        return (user_id, password)
    #userテーブルにuser_idが存在するかチェック
    def exists_user(self, user_id):
            sql = "SELECT COUNT(*) FROM {0} WHERE user_id = '{1}'".format(self.USER_TBL, user_id)
    ...
    def issue_user(self, user_id, password, issuer_id):
        ...
        expiration_time = stringutils.format_time(now + self.DEFAULT_EXPIRATION)
        sql = "INSERT INTO {0} (user_id, password, access_issuer_id, issuance_time, expiration_time) VALUES('{1}', '{2}', '{3}', '{4}', '{5}')".format(self.USER_TBL, user_id, password, issuer_id, issuance_time, expiration_time)
        self._execute(sql)
        logger.info("issue user: %s", user_id)
        return (now, now + self.DEFAULT_EXPIRATION)

    def check_issuer(self, issuer_id, password):
        sql = "SELECT password FROM {0} WHERE issuer_id = '{1}'".format(self.ISSUER_TBL, issuer_id)
